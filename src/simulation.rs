...
mod tests {
    use super::*;
    use crate::map_generation::Tile;

    fn empty_grid(width: usize, height: usize) -> Vec<Tile> {
    ...
    #[test]
    fn bfs_returns_first_step_on_straight_path() {
        let tiles = empty_grid(5, 1);
        assert_eq!(bfs(&tiles, 5, 1, (0, 0), (4, 0)), Some((1, 0)));
    }

    #[test]
    fn bfs_returns_none_when_already_at_target() {
        let tiles = empty_grid(5, 5);
        assert_eq!(bfs(&tiles, 5, 5, (2, 2), (2, 2)), None);
    }

    #[test]
    fn bfs_reaches_adjacent_target_in_one_step() {
        let tiles = empty_grid(3, 3);
        assert_eq!(bfs(&tiles, 3, 3, (1, 1), (2, 1)), Some((2, 1)));
    }

    #[test]
    fn bfs_routes_around_obstacles() {
        // Grille 3x3 : obstacles en (0,1) et (1,1)
        // De (0,0) à (0,2) le chemin doit passer par la colonne droite
        let mut tiles = empty_grid(3, 3);
        tiles[1 * 3 + 0] = Tile::Obstacle; // (0,1)
        tiles[1 * 3 + 1] = Tile::Obstacle; // (1,1)
        assert_eq!(bfs(&tiles, 3, 3, (0, 0), (0, 2)), Some((1, 0)));
    }

    #[test]
    fn bfs_returns_none_when_target_is_unreachable() {
        // (0,0) bloqué immédiatement : ses deux voisins sont des obstacles
        let mut tiles = empty_grid(3, 3);
        tiles[0 * 3 + 1] = Tile::Obstacle; // (1,0)
        tiles[1 * 3 + 0] = Tile::Obstacle; // (0,1)
        assert_eq!(bfs(&tiles, 3, 3, (0, 0), (2, 2)), None);
    }

    // --- random_step ---

    #[test]
    fn random_step_returns_none_when_all_neighbors_are_obstacles() {
        // Grille 3x1 : Empty Obstacle Empty — robot en (0,0), seul voisin est un obstacle
        let mut tiles = empty_grid(3, 1);
        tiles[1] = Tile::Obstacle;
        let mut rng = rand::thread_rng();
        assert_eq!(random_step(0, 0, 3, 1, &tiles, &mut rng), None);
    }

    #[test]
    fn random_step_returns_adjacent_non_obstacle_cell() {
        let tiles = empty_grid(3, 3);
        let mut rng = rand::thread_rng();
        let result = random_step(1, 1, 3, 3, &tiles, &mut rng);
        assert!(result.is_some());
        let (nx, ny) = result.unwrap();
        let dist = (nx as isize - 1).unsigned_abs() + (ny as isize - 1).unsigned_abs();
        assert_eq!(dist, 1, "le prochain pas doit être à exactement 1 case de (1,1)");
    }

    #[test]
    fn random_step_never_returns_obstacle_position() {
        // Grille 3x3, obstacles à droite et en bas de (1,1) → seuls gauche et haut sont valides
        let mut tiles = empty_grid(3, 3);
        tiles[1 * 3 + 2] = Tile::Obstacle; // (2,1)
        tiles[2 * 3 + 1] = Tile::Obstacle; // (1,2)
        let mut rng = rand::thread_rng();

        for _ in 0..50 {
            if let Some((nx, ny)) = random_step(1, 1, 3, 3, &tiles, &mut rng) {
                assert_ne!(tiles[ny * 3 + nx], Tile::Obstacle, "random_step ne doit jamais retourner une case obstacle");
            }
        }
    }
}
